...
public class DbUtilsTest extends TestCase {
     ...
     * Logger.
     */
    private static final Logger log = Logger.getLogger(DbUtilsTest.class.
            getName());
    ...
    public final List<DbUtilsMovie> generateData(final int maxItems) {
        List<DbUtilsMovie> dataList = new ArrayList<>();
        for (int i = 0; i < maxItems; i++) {
            dataList.add(new DbUtilsMovie(randomString(CHARS, 20), randomString(CHARS, 50), 1950 + (int) (Math.random() * 67)));
        }
        return dataList;
    }
    ...
    public final void test() throws Exception {
        ...
        DbUtilsMovies movies = (DbUtilsMovies) context.lookup("java:global/jpavsdbutils/DbUtilsMovies");
        // Add movies
        long startTime = System.nanoTime();
        List<DbUtilsMovie> dataList = generateData(MAX_ITEMS);
        for (DbUtilsMovie movie : dataList) {
        ...
        List<DbUtilsMovie> list = movies.getMovies();
